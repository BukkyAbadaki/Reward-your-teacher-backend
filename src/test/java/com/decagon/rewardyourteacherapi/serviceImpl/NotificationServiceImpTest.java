...
//    String fundedWallet = "You have successfully funded you wallet with N";
    ...
    void saveTransactionNotification() throws NullPointerException {
        ...
        Notification  not = notificationRepository.findByMessageAndUser(MessageType.TEACHER_REWARDED.getStr()+user1.getName(), user2).orElse(null);
        assertTrue(not.getMessage().contains(MessageType.TEACHER_REWARDED.getStr()));
        assertNotNull(notificationRepository.findByMessageAndUser(MessageType.REWARD_TEACHER.getStr()+user2.getName(), user1).orElse(null));
        Notification not2 = notificationRepository.findByMessageAndUser(MessageType.REWARD_TEACHER.getStr()+user2.getName(), user1).orElse(null);
        ...
        String formatMoney = df.format(transaction2.getAmount());
        System.out.println(notificationRepository.findByMessageAndUser(MessageType.FUNDED_WALLET.getStr(), user1));
        assertNotNull(notificationRepository.findByMessageAndUser(MessageType.FUNDED_WALLET.getStr() + formatMoney, user1));
        not = notificationRepository.findByMessageAndUser(MessageType.FUNDED_WALLET.getStr() + formatMoney, user1).orElse(null);
